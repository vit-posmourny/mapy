<div class="w-1/4 shadow-2xl p-8 text-xl font-medium overflow-hidden min-w-96">
    ...
    <form   wire:submit.prevent="store" class="flex flex-col">
        ...
        <div class="grid grid-cols-7 items-baseline mb-4">
            <span class="select-none">Lat:</span><x-text-input id="inputLatitude" wire:model="latitude" name="latitude" placeholder="Enter Latitude" class="col-span-6"/>
        </div>

            <x-input-error class="self-start ml-14" :messages="$errors->get('latitude')"/>

        <div class="grid grid-cols-7 items-baseline my-4">        
            <span class="select-none">Lon:</span><x-text-input id="inputLongitude" wire:model="longitude" name="longitude" placeholder="Enter Longitude" class="col-span-6"/> 
        </div>
