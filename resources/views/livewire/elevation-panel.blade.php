<div class="w-1/4 shadow-2xl p-8 text-xl font-medium overflow-hidden min-w-96">
    ...
    <form wire:submit.prevent="store" class="flex flex-col">
        ...
        <div class="grid grid-cols-12 items-baseline mb-4">
            <span class="select-none col-span-3">z.šířka:</span><x-text-input id="i-Latitude" wire:model="latitude" name="latitude" placeholder="šířka" class="col-span-9"/>
        </div>
        
            <x-input-error class="self-start ml-14" :messages="$errors->get('latitude')"/>

        <div class="grid grid-cols-12 items-baseline my-4">        
            <span class="select-none col-span-3">z.délka:</span><x-text-input id="i-Longitude" wire:model="longitude" name="longitude" placeholder="délka" class="col-span-9"/> 
        </div>

            <x-input-error class="self-start ml-14" :messages="$errors->get('longitude')"/>

        <div class="grid grid-cols-8 items-baseline my-4">
            <span class="select-none col-span-3">nadm.výška:</span><x-text-input id="i-Elevation" wire:model="elevation" name="elevation" placeholder="výška" class="custom_input col-span-4"/><span class="select-none col-span-1 text-right">m</span>
        </div>
